...
def current_host(request: Request | None = None, websocket: WebSocket | None = None) -> str:
    ...
    if request is not None:
        ...
        raw = websocket.headers.get("host", "")
    if not raw:
        return "127.0.0.1"
...
async def vless_ws(websocket: WebSocket):
    ...
    try:
        ...
        if first.get("type") != "websocket.receive" or not first.get("bytes"):
            await send_page(websocket, 1003, "Binary VLESS data required")
            return
        request = parse_vless_request(first["bytes"])
    ...
    except (WebSocketDisconnect, asyncio.IncompleteReadError, ConnectionResetError, BrokenPipeError):
        pass
    except Exception:
        try:
...
<div class="row"><button class="primary" onclick="copyLink()">Copy VLESS link</button><button onclick="location.href='/api/status'">Check status</button></div>
<p class="muted" style="font-size:13px;margin-top:16px">This server accepts VLESS over WebSocket and uses the panel's generated UUID.</p>
</div></main><script>function copyLink(){navigator.clipboard.writeText(document.getElementById('uri').value).then(()=>alert('Copied'))}</script></body></html>'''
